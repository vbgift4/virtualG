document.addEventListener('DOMContentLoaded', ()=>{
  ...
  function showPage(id){
    ...
    pages.forEach(p=> scrollMainTop(p));
    stopConfetti();
  ...
  heroTap.addEventListener('touchstart', ()=>{ if(!musicPlaying && bgAudio){ bgAudio.play().catch(()=>{}); musicPlaying=true;} }, {once:true});
  ...
  function setCollapsibleDefault(){
    collapsibles.forEach(c=>{
      ...
      if(breakpoint.matches){
        ...
        toggle.addEventListener('click',()=>{ const isOpen=c.getAttribute('data-open')==='true'; c.setAttribute('data-open',String(!isOpen)); toggle.setAttribute('aria-expanded',String(isOpen)); if(!isOpen){ const page=c.closest('.page'); if(page) page.scrollTo({top:0,behavior:'smooth'}); }});
      }else{
  ...
  setCollapsibleDefault(); breakpoint.addEventListener('change', setCollapsibleDefault);
